...
import { DELETE_FABRIC_REQUEST, DELETE_FABRIC_SUCCESS, DELETE_FABRIC_FAILURE } from '../actions/fabricsActions';
...
export default function fabrics(state, action) {
  switch (action.type) {
    ...
    case GET_FABRICS_SUCCESS:
      let getListSuccess = state.setIn(['list', 'fabrics'], fromJS(action.fabrics));
    ...
    case DELETE_FABRIC_REQUEST:
      return state.setIn(['loading'], true);
    case DELETE_FABRIC_SUCCESS:
      return state.setIn(['loading'], false);
    case DELETE_FABRIC_FAILURE:
      return state.setIn(['loading'], false);
  }
  return state;
}